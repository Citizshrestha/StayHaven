...
import { MapPin, Clock, ChefHat, CheckCircle, AlertCircle, X, Calendar } from "lucide-react";
...
const AssignedAreas = ({ orders = [], onFilterByArea, onClose, isDarkMode = false }) => {
  ...
  const getStatusColor = (area) => {
    if (area.readyOrders > 0) return '#10B981';
    if (area.preparingOrders > 0) return '#F59E0B';
    if (area.pendingOrders > 0) return '#3B82F6';
    return '#9CA3AF';
  };
